...
val print_num = (fn x => print(Real.toString(Real.fromInt(x))))
...
fun substitute subst =
  let fun subst_rec (Term(oper,sons)) = Term(oper, map subst_rec sons)
        | subst_rec ((Var (n)))     = (assoc n subst) handle Failure (_) => Var (n)
  in subst_rec
  end
...
fun pretty_term x =
let
  fun pretty_close (Term(oper, m)) =
        if mem oper INFIXES then
          (print "("; pretty_term (Term(oper, m)); print ")")
        else pretty_term (Term(oper, m))
  ...
  case x of Var(n) =>
     ...
     | (Term (oper,sons)) =>
      if mem oper INFIXES then
        case sons of
            [s1,s2] =>
              (pretty_close s1; print oper; pretty_close s2)
          | _ =>
       ...
       (print oper;
        case sons of
          ...
          | t::lt =>(print "(";
                     pretty_term t;
                     app (fn t => (print ","; pretty_term t)) lt;
                     print ")"))
end
...
fun pretty_rule (k,(n,(M,N))) =
 (print_num k; print " : ";
  pretty_term M; print " = "; pretty_term N;
  print_newline())
...
fun lex_ext order ((Term(m,sons1)), (Term(n,sons2))) =
      let fun lexrec ([] , []) = Equal
            | lexrec ([] , _ ) = NotGE
            | lexrec ( _ , []) = Greater
            | lexrec (x1::l1, x2::l2) =
                case order (x1,x2) of
                  Greater => if for_all (fn N' => gt_ord order (Term(m, sons1),N')) l2
                             then Greater else NotGE
                | Equal => lexrec (l1,l2)
                | NotGE => if exists (fn M' => ge_ord order (M',Term(n, sons2))) l1
                           then Greater else NotGE
...
fun deletion_message (k,_) =
  (print "Rule ";print_num k; message " deleted")

(* Generate failure message *)
fun non_orientable (M,N) =
  (pretty_term M; print " = "; pretty_term N; print_newline())
